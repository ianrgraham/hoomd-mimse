...
for k in range(len(postfix_arr)):
    ...
    globals()[f'distances_arr_{postfix_arr[k]}'] = np.array(globals()[f'distances_arr_{postfix_arr[k]}'])
        ...
        # Calculate CDF
    globals()[f'sorted_dist_{postfix_arr[k]}'] = np.sort(globals()[f'distances_arr_{postfix_arr[k]}'])
    # print('sorted distances',sorted_distances_A)
    globals()[f'cdf_{postfix_arr[k]}'] = np.arange(1, len(globals()[f'sorted_dist_{postfix_arr[k]}']) + 1) / len(globals()[f'sorted_dist_{postfix_arr[k]}'])

    print('len cdf',len(globals()[f'cdf_{postfix_arr[k]}']))
    print('len sorted distances',len(globals()[f'sorted_dist_{postfix_arr[k]}']))
